...
function mise
  if test (count $argv) -eq 0
    command /home/angel/.local/bin/mise
    return
  end
  ...
  if contains -- --help $argv
    command /home/angel/.local/bin/mise "$command" $argv
    return $status
  end
  ...
  case deactivate shell sh
    ...
    if contains -- -h $argv
      ...
      source (command /home/angel/.local/bin/mise "$command" $argv |psub)
    end
  case '*'
    command /home/angel/.local/bin/mise "$command" $argv
  end
...
function __mise_env_eval --on-event fish_prompt --description 'Update mise environment when changing directories';
    /home/angel/.local/bin/mise hook-env -s fish | source;
...
function fish_command_not_found
    if string match -qrv -- '^(?:mise$|mise-)' $argv[1] &&
        /home/angel/.local/bin/mise hook-not-found -s fish -- $argv[1]
        /home/angel/.local/bin/mise hook-env -s fish | source
